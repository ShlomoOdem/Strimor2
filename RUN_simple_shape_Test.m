...
for ii = 1:numel(SortedTestFile)
    % if str2double(cell2mat(extractBetween(SortedTestFile(ii).name,"N_","_"))) == 100
        ...
        for k = 0:(numel(Exp)/Num_Problem_In_Batch)
            ...
            for jj = 1:50
                ...
                [Tree, error,msg] = SimpleShapeAlgorithm(BasicWS, N, StartNode, TargetNode,"GroupSize_Approx",Group_Approx);
    ...
    if Group_Approx
        save(fullfile(SolutionFolder,join(["N",string(N),"Approx.mat"])),"Solution");
    else
